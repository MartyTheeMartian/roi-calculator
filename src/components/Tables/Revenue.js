...
import { addRevenueItem, deleteRevenueItem } from '../../actions';
...
class Revenue extends Component {
  ...
  addItem = () => {
    const itemNameArray = this.props.revenueList.map((item) => {
    ...
    if (!this.state.itemName || !this.state.oneTime || !this.state.monthly) {
      ...
      this.props.addRevenueItem(item);
      this.setState({
        ...this.state,
        validationMsg: ''
      });
  ...
  deleteItem = (index) => {
    this.props.deleteRevenueItem(index);
  }

  displayDynamicRows = () => {
    return this.props.revenueList.map((item, index) => (
      <tr key={index}>
        <td>{item.itemName}</td>
      ...
      <div>
        <table className="table table-bordered">
          ...
          <tbody>
            ...
            <tr className="item-submit">
              <td>
                <input type="text" placeholder="Item Name" value={this.state.itemName} onChange={this.updateItemName}/>
              </td>
              <td>
              ...
              <td >
                <button className="btn btn-primary btn-sm" onClick={this.addItem}>Add Revenue Item</button>
              </td>
            </tr>
          </tbody>
        </table>
...
const mapStateToProps = (state) => {
  return { revenueList: state.revenueReducer };
};

const mapDispatchToProps = (dispatch) => {
  return bindActionCreators({ addRevenueItem, deleteRevenueItem}, dispatch);
};
